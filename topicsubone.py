...
def on_message(client,userdata,message):
    ...
    try:
        ...
        if (len(meslist) == 3): # 데이터와 명령어 구분 (3개시 데이터)
            createFolder('/media/kmuscrc/237C250608A0AFEA/'+fileway)

            if os.path.isfile('/media/kmuscrc/237C250608A0AFEA/'+thefile) and thefile not in filenum and thefile not in senddone: # 파일이 존재할 때
                print("파일이 존재")
                senddone[thefile] = False #파일이 존재하고 딕셔너리에 없을 때
                with open('/media/kmuscrc/237C250608A0AFEA/'+thefile) as myfile: # 파일 열어서 몇 번째 줄까지 써져있는지 확인
                ...
                perform[thefile] = [receivedTime.strftime('%Y/%m/%d'),receivedTime.strftime('%H:%M:%S.%f'),0,0,0,receivedTime.strftime('%H:%M:%S.%f')]
                restartcheck[thefile] = False
                print(perform)
            ...
            if (int(meslist[1]) == filenum[thefile]): # 행수가 같으면
                ...
                if meslist[2] == 'DONE':
                    ...
                    time_1 = datetime.strptime(perform[thefile][5],'%H:%M:%S.%f')
                    time_2 = datetime.strptime(receivedTime,'%H:%M:%S.%f')
                    print(time_1,time_2)
                    perform[thefile][5] = time_2 -time_1
                    print(perform)
                    print("걸린시간",perform[thefile][5])
                    df = pd.DataFrame(perform)
                    print(df)
                    df = df.transpose()
                    df.rename(columns=['실험 일자','받은 시간','stop 받은 횟수','restart 받은 횟수','재전송 횟수','끝난 시각','총 소요시간'], inplace=True)	# 행열이 전환된 데이터프레임의 열 이름 제대로 수정
                    # df = df.drop(df.index[0])
                    print(df)
                    df.to_excel('perform.xlsx',index = False)
                else:
                    # 파일에 저장 
                    filenum[thefile] += 1
                    f = open('/media/kmuscrc/237C250608A0AFEA/'+thefile, 'a')
                    f.write(meslist[2])
                    f.close() 
            else:
