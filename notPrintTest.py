    ...
    try:
        ...
        if (len(meslist) == 3): # 데이터와 명령어 구분 (3개시 데이터)
            ...
            if os.path.isfile('/media/kmuscrc/237C250608A0AFEA/'+thefile) and thefile not in filenum and thefile not in senddone: # 파일이 존재할 때
                ...
                perform[thefile] = [receivedTime.strftime('%Y/%m/%d'),receivedTime.strftime('%H:%M:%S.%f'),0,0,0,receivedTime.strftime('%H:%M:%S.%f'),receivedTime.strftime('%H:%M:%S.%f')]
                restartcheck[thefile] = False
                # print(perform)
            ...
            if (int(meslist[1]) == filenum[thefile]): # 행수가 같으면
                ...
                if meslist[2] == 'DONE':
                    ...
                    perform[thefile][5] = receivedTime
                    time_1 = datetime.strptime(perform[thefile][1],'%H:%M:%S.%f')
                    time_2 = datetime.strptime(receivedTime,'%H:%M:%S.%f')
                    # print(time_1,time_2)
                    perform[thefile][6] = time_2 -time_1
